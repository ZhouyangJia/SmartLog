...
extract(){
	echo "SmartLog extracts compile command ..."
	`./extract_command.pl ../compile_commands.json`
	check
}

build(){
	echo "SmartLog builds LLVM IR ..."
	`./build_ir.sh`
	check
}

call_dependence(){
    echo "SmartLog finds call dependence ..."
    ...
    for file in ${arr[*]}
    do
        echo -e "[1/2][$count/$num] Now analyze the file: $file" >&2;
        count=`expr $count + 1`;
        `$LLVM_BIN/opt -load $LLVM_LIB/LLVMCallDependence.dylib -call-dependence $file > /dev/null`;
        check
    done
}

logging_behavior(){
	echo "SmartLog finds logging behavior ..."
	`rm -rf logging_statement.out logging_behavior.out`;
	`cat ./compiled_files.def | xargs $LLVM_BIN/clang-smartlog -find-logging-behavior`
	check
}

logged_ast(){
	echo "SmartLog finds logging behavior ..."
	`rm -rf logged_snippet_ast.out`;
	`cat ./compiled_files.def | xargs $LLVM_BIN/clang-smartlog -find-logged-snippet`
	check
}
